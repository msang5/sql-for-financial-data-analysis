# Financial Reporting and Analysis System using SQL

## Project Description

**Overview:**
The project aims to develop a comprehensive financial reporting and analysis system using SQL (Structured Query Language). The system utilizes five key datasets: general ledger, charts of accounts, territory, calendar, and cash flow. By leveraging these datasets, the system generates essential financial reports including Profit and Loss (P&L) statements, Balance Sheets, and Cash Flow statements.

## Project Goal

Design SQL queries to generate accurate and timely financial reports including Profit and Loss statements, Balance Sheets, and Cash Flow statements.

## Tools and Library Used

[<img src="./image/azure-logo.jpg" alt="azure-logo" width="170"/>](https://azure.microsoft.com/en-us/products/data-studio/)

## Project Result

[Click here to access the SQL source code](./SQLQuery_finance.sql)

### A. Profit and Loss Statement

**Create pivot table report of profit and loss for 2018, 2019, 2020**
<img src="./image/pl2.png" alt="" width = "550"/>

**Comparison of SQL Query to Excel result**
<img src="./image/excel_pl1.png" alt="" width = "400"/>

**Create pivot table report of profit and loss for 2018, 2019, 2020 for country France**
<img src="./image/pl3.png" alt="" width = "550"/>

**Comparison of SQL Query to Excel result for France only**
<img src="./image/excel_pl2.png" alt="" width = "400"/>

**Calculating Profit and Loss Statement Related Values**
<img src="./image/excel_pl_all.png" alt="" width = "400"/>

**Comparison of SQL Query to Excel Profit and Loss Statement Related Values**
<img src="./image/excel_pl_all.png" alt="" width = "400"/>

### B. Balance Sheet

**Create pivot table report of balance sheet for 2018, 2019, 2020**
<img src="./image/bs1.png" alt="" width = "700"/>

**Comparison of SQL Query to Excel result**
<img src="./image/excel_bs1.png" alt="" width = "500"/>

**Calculating Balance Sheet Related Values**
<img src="./image/bs_all.png" alt="" width = "800"/>

**Comparison of SQL Query to Balance Sheet Excel result**
<img src="./image/excel_bs_all.png" alt="" width = "450"/>

### C. Calculating Ratios

<img src="./image/ratio1.png" alt="" width = "700"/>
<img src="./image/ratio2.png" alt="" width = "270"/>
<img src="./image/ratio3.png" alt="" width = "500"/>

### D. Cash Flow Statement
**Calculating Cash Flow Related Values**
<img src="./image/CF.png" alt="" width = "500"/>

**Comparison of SQL Query to Cash Flow Excel result**
<img src="./image/excel_CF.png" alt="" width = "450"/>

---

## About the Maintainer

**Manmohan S.** is a Supply Chain Analyst with over 4 years of experience in inventory planning, logistics coordination, and operational analytics. He specializes in transforming complex datasets into actionable insights using SQL, Python, and Power BI. This project serves as a framework for automated financial and operational reporting within enterprise environments.

**Contact Information:**
- Email: manmohansangola1@gmail.com
- LinkedIn: https://www.linkedin.com/in/manmohan-sangola/